...
var (
	sgsTotalCount   = NewOperatorEventGauge("clusters", "Total number of clusters managed")
	sgsCreatedCount = NewOperatorEventCounter("clusters_created", "Total number of clusters created")
	sgsDeletedCount = NewOperatorEventCounter("clusters_deleted", "Total number of clusters deleted")
)

// RegisterMetrics registers the cluster CRUD metrics.
func RegisterMetrics() {
	RegisterOperatorMetric(sgsTotalCount)
	RegisterOperatorMetric(sgsCreatedCount)
	RegisterOperatorMetric(sgsDeletedCount)
}
...
func NewOperatorEventCounter(name string, help string) *prometheus.CounterVec {
        return newCounter("sg_operator", "cluster", name, help, []string{"podName"})
}
...
func NewOperatorEventGauge(name string, help string) *prometheus.GaugeVec {
        return newGauge("sg_operator", "cluster", name, help, []string{"podName"})
}
...
func NewAgentEventCounter(name string, help string) *prometheus.CounterVec {
        return newCounter("sg", "innodb", name, help, []string{"podName", "clusterName"})
}
...
func NewAgentStatusCounter(name string, help string) *prometheus.CounterVec {
        return newCounter("sg", "innodb", name, help, []string{"podName", "clusterName", "instanceStatus"})
}
...
func IncStatusCounter(counter *prometheus.CounterVec, status string) {
...
func statusLabels(status string) prometheus.Labels {
